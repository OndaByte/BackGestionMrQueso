...
public class VentaControl {/*
	    @Autowired
	    private VentasDAO dao = new VentasDAOImp();
	    @Autowired
	    private CajaDAO cajadao= new CajaDAOImp();
	    @Autowired
	    private ProductosDAO proddao = new ProductosDAOImp();
	    */
	    ...
	    public static void nuevaVenta(Context ctx){
	    	...
	    	Venta n = objectMapper.readValue(ctx.body(), Venta.class);
	        controlesVenta(n);
...
//	            imageWrapper.setJustification(EscPosConst.Justification.Center);
	    ...
	    public static void controlesVenta(Venta n) throws Exception {
	        ...
	        List<ProductoItem> itemsAux = new ArrayList<>();
	        itemsAux.addAll(n.getItems());
	        controlStock(itemsAux);
	        controlTotal(itemsAux,n.getTotal());
	    }

	    public void controlStock(List<ProductoItem> prods) throws Exception {
	        Productos aux;
	        for(ProductoItem p : prods){
	            aux = proddao.getProducto(p.getProducto_id());
	            if(aux.getStock_actual() - p.getCantidad() < 0) {
	                throw new NumberOutOfRange("Se excedio el stock del producto '"+aux.getNombre()+"'.\n");
	            }
	        }
	    }
	}
